...
export function LoginForm({ onSubmit, isBusy }: LoginFormProps) {
  ...
  return (
    <div className="w-full min-w-0 bg-linear-to-br from-orange-50 to-yellow-50 border-3 sm:border-4 border-amber-900 shadow-2xl p-2.5 sm:p-6 lg:p-7 xl:p-8 overflow-hidden pixel-card" style={{ boxShadow: '0 10px 0 rgba(0,0,0,0.15), 0 14px 28px rgba(0,0,0,0.25)' }}>
      {/* Header Section */}
      <div className="mb-4 sm:mb-6 lg:mb-7 space-y-2 sm:space-y-3">
        <div className="text-center">
          <div className="mb-2 flex justify-center">
            <div className="h-11 w-11 sm:h-14 sm:w-14 lg:h-16 lg:w-16 bg-linear-to-br from-orange-400 to-orange-500 border-3 border-orange-900 rounded-lg flex items-center justify-center" style={{ boxShadow: '4px 4px 0px rgba(139,69,19,0.5)' }}>
              <Shield className="h-5 w-5 sm:h-6 sm:w-6 lg:h-7 lg:w-7 text-white" />
            </div>
          </div>
          <h1 className="text-lg sm:text-3xl lg:text-4xl font-pixel text-orange-900 tracking-[0.04em] sm:tracking-widest leading-tight" style={{ textShadow: '2px 2px 0px rgba(139,69,19,0.2)' }}>
            WORKHERO
          </h1>
          <p className="text-sm sm:text-lg lg:text-xl font-jersey text-amber-700 mt-1">
            Barangay Kusina
          </p>
      ...
      <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-5 lg:space-y-6">
        ...
        {error && (
          <div className="bg-red-100 border-3 border-red-600 p-3 sm:p-4 pixel-box animate-shake">
            <p className="text-red-900 font-pixel text-[11px] sm:text-xs lg:text-sm">
              {error}
            </p>
          </div>
        )}
