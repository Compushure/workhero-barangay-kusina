    ...
    <div className="space-y-4">
      {users.map((user) => {
        ...
        return (
          <div
            key={user.id}
            className="bg-background-soft rounded-xl border border-accent/25 p-3 hover:shadow-lg transition-shadow duration-300 shadow-sm/25"
          >
            <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4">
              ...
              <div className="shrink-0 flex items-center gap-3 min-w-0 sm:min-w-48">
                ...
                <div className="min-w-0 flex-1 sm:w-48">
                  ...
                  <p className="text-2xs text-secondary truncate">{user.employee_id}</p>
                </div>
              </div>
                      ...
                      return (
                        <div key={badgeId} className="group relative">
                          ...
                          <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:block z-10">
                            <div className="bg-background text-primary text-2xs rounded px-1.5 py-0.5 whitespace-nowrap border-b-2 border-x border-accent-secondary/25">
                              {badge?.name}
                            </div>
                          </div>
                        </div>
