...
CREATE OR REPLACE FUNCTION "public"."get_leaderboard_as_of"("p_cutoff" timestamp with time zone) RETURNS TABLE("user_id" "uuid", "user_name" "text", "performance_score" bigint, "total_kpi_points" bigint, "badge_points" bigint, "task_count" bigint, "rank" bigint)
    LANGUAGE "sql" STABLE SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$WITH badge_points AS (
  SELECT
    ub.awarded_to AS user_id,
    COALESCE(SUM(b.points), 0)::bigint AS badge_points
  FROM "UserBadges" ub
...
CREATE TABLE IF NOT EXISTS "public"."User" (
    ...
    CONSTRAINT "User_level_max_cap_check" CHECK (("level" <= 10)),
...
CREATE TABLE IF NOT EXISTS "public"."Dishes" (
    ...
    "start_appear_level" integer DEFAULT 1,
    "img_link" "text"
);
...
COMMENT ON COLUMN "public"."KPITask"."verification_requested_at" IS 'Timestamp when employee submits the task for verification (in review). Updated on each new submission.';
...
COMMENT ON COLUMN "public"."Level"."description" IS 'Description text for this level (formerly perk).';
...
ALTER TABLE "public"."User" DISABLE TRIGGER "trg_user_award_xp_and_level";
...
ALTER TABLE "public"."User" DISABLE TRIGGER "user_total_xp_insert_trg";
...
ALTER TABLE "public"."User" DISABLE TRIGGER "user_total_xp_update_trg";
